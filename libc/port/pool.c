...
enum {
	NOT_MAGIC = 0xdeadfa11ul,
	DEAD_MAGIC = 0xdeaddeadul,
};
...
enum {
	ARENA_MAGIC = 0xC0A1E5CEU + 1,
	ARENATAIL_MAGIC = 0xEC5E1A0CU + 1,
};
...
poolnewarena(Pool *p, ulong asize)
{
	...
	b = (Alloc*)A2B(a);
	b->magic = UNALLOC_MAGIC;
	blocksetsize((Bhdr *) b, (uchar*)A2TB(a)-(uchar*)b);
	blockcheck(p, (Bhdr *) b);
	pooladd(p, (Bhdr *) b);
	blockcheck(p, (Bhdr *) b);
...
arenamerge(Pool *p, Arena *bot, Arena *top)
{
	...
	blockcheck(p, (Bhdr *) bot);
	blockcheck(p, (Bhdr *) top);
