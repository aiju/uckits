...
(sed -n 's/^#define RCC_\([A-Z0-9]\+\)ENR\([0-9]*\)_GPIO\([A-Z]\)EN .*$/\1;\2;\3/gp' $H | awk -F';' ' { printf "\tcase PERGPIO%s:\n\t\tif(st > 0) RCC->%sENR%s |= RCC_%sENR%s_GPIO%sEN;\n\t\telse if(st == 0) RCC->%sENR%s &= ~RCC_%sENR%s_GPIO%sEN;\n\t\treturn %shz;\n", $3, $1, $2, $1, $2, $3, $1, $2, $1, $2, $3, tolower($1) } ' | grep .) || \
(sed -n 's/^#define RCC_\([A-Z0-9]\+\)ENR\([0-9]*\)_IOP\([A-Z]\)EN .*$/\1;\2;\3/gp' $H | awk -F';' ' { printf "\tcase PERGPIO%s:\n\t\tif(st > 0) RCC->%sENR%s |= RCC_%sENR%s_IOP%sEN;\n\t\telse if(st == 0) RCC->%sENR%s &= ~RCC_%sENR%s_IOP%sEN;\n\t\treturn %shz;\n", $3, $1, $2, $1, $2, $3, $1, $2, $1, $2, $3, tolower($1) } ')
